    ...
    <Card className="@container/card">
      ...
      <CardFooter className="flex-col items-start gap-1.5 text-sm">
        {trend && (
          ...
          >
            {trend.positive
              ? "Менше ніж минулий місяць"
              : "Більше ніж минулий місяць"}
            {trend.positive ? (
      ...
      <PopoverContent className="w-auto p-0 rounded-2xl" align="start">
        ...
        <div className="flex gap-2 p-3 border-t border-gray-100 dark:border-zinc-800">
          ...
          <Button
            size="sm"
            variant="ghost"
            className="rounded-xl text-xs text-gray-400 cursor-pointer"
...
export default function ExpensesPage() {
      ...
      expenses.filter((e) => {
        if (dateRange?.from || dateRange?.to) {
          const d = parseISO(e.date),
            ...
            to = dateRange.to ?? new Date(9999, 11, 31);
  ...
  const trendPct =
    prevMonth > 0
      ? (((currMonth - prevMonth) / prevMonth) * 100).toFixed(1)
      : null;
      ...
      <div className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:grid-cols-3">
        <MetricCard
          ...
          trend={
            trendPct
              ? {
                  value: `${Math.abs(Number(trendPct))}% vs минулий місяць`,
                  positive: Number(trendPct) <= 0,
                }
              : undefined
          }
        />
        <MetricCard
          ...
          value={`${prevMonth.toFixed(0)} ₴`}
          sub="для порівняння"
        />
        <MetricCard
          label="За обраний період"
          value={`${totalFiltered.toFixed(0)} ₴`}
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Select value={filterCat} onValueChange={setFilterCat}>
            <SelectTrigger className="h-9 rounded-xl text-sm w-40">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
