...
import { ArrowLeft, Plus, Trash, Package, Tag, Layers, CheckCircle2 } from "lucide-react";
...
type ProductForm = {
  ...
  isActive: boolean; categoryId: string; variants: VariantForm[];
};
...
export default function EditProductPage() {
  ...
  const [product, setProduct]   = useState<ProductForm>({
    ...
    isActive: true, categoryId: "", variants: [],
  });
      ...
      .then(([p, cats]) => {
        ...
        setProduct({
          ...
          variants:    (p.variants ?? []).map((v: any) => ({
            id: v.id, size: v.size, color: v.color, stock: String(v.stock),
          })),
        });
      })
  ...
  const addVariant = useCallback(() => {
    setProduct((p) => ({ ...p, variants: [...p.variants, { size: "", color: "", stock: "" }] }));
  }, []);

  const updateVariant = useCallback((index: number, key: keyof VariantForm, value: string) => {
    setProduct((p) => {
  ...
  const step3Done = product.variants.length > 0 &&
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950">
      <div className="max-w-2xl mx-auto px-4 py-6 flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold tracking-tight">Редагувати продукт</h1>
            <p className="text-xs text-gray-400 mt-0.5">Змініть потрібні поля та збережіть</p>
          </div>
        </div>

        {/* ── Крок 1: Назва та артикул ── */}
        <Section step={1} title="Назва та артикул" icon={<Tag size={16} />} done={step1Done}>
        ...
        <Section step={2} title="Ціна та категорія" icon={<Package size={16} />} done={step2Done}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Field label="Кількість на складі" required>
              <Input
                ...
                title={product.variants.length > 0 ? "Розраховується автоматично з варіантів" : ""}
              />
              {product.variants.length > 0 && (
                <p className="text-xs text-gray-400 -mt-1">
                  Автоматично = сума варіантів:{" "}
                  <span className="font-semibold">
                    {product.variants.reduce((s, v) => s + (parseInt(v.stock) || 0), 0)} шт
                  </span>
                </p>
              )}
            </Field>
            <Field label="Категорія" required>
              <Select
                ...
                onValueChange={(v) => setProduct((p) => ({ ...p, categoryId: v }))}
              >
            ...
            <Field label="Статус">
              <Select
                ...
                onValueChange={(v) => setProduct((p) => ({ ...p, isActive: v === "true" }))}
              >
                <SelectTrigger className="rounded-xl cursor-pointer">
                  <SelectValue />
                </SelectTrigger>
        ...
        {/* ── Крок 3: Варіанти ── */}
        <Section step={3} title="Варіанти" icon={<Layers size={16} />} done={step3Done}>
          <p className="text-xs text-gray-400 -mt-1">Розмір, колір та кількість для кожного варіанту</p>
